...
<html>
			...
			<div class="container">
				...
				<h4>
					<?php
						...
						{
							$id_pilote = $row['id'];
							// echo "numéro pilote = ".$id_pilote;
						}
						
						// On calcule le nombre kimoètres parcourus et le nombre d'étapes et on l'affiche
						...
						while($row = mysqli_fetch_assoc($result))
						{
							...
							$etapes = $row['etapes'];
							if ( $km > 0) {
								echo $km." KM PARCOURUS CETTE SAISON EN ".$etapes." ÉTAPES.";
								} else {
								echo "VOUS N'AVEZ PAS ENCORE ENREGISTRÉ D'ÉTAPE";
							}
						}
				...
				<h4>
					...
					<a href='#Changemoto' role='button' data-toggle='modal' data-bs-toggle='tooltip' data-bs-placement='top' title='Changer de moto'>
					<img src='img/rally_edit-icon-NB.svg'class='img-fluid' style='width:25px;'></a>
					
					<!-- Modal CHANGER MOTO -->
					<div class='modal fade' id='Changemoto' tabindex='-1' role='dialog' aria-labelledby='ModalChange' aria-hidden='true'>
						<div class='modal-dialog modal-sm' role='document'>
							<div class='modal-content'>
								<div class='modal-header'>
									<h5 class='modal-title text-primary' id='exampleModalLabel'>Changer de moto</h5>
								</div>
								<FORM action='change_moto.php?id_user=<?php echo $_SESSION["id"];?>' method='post' name='change-moto' charset='UTF-8' class='needs-validation'>
								<div class='modal-body text-primary fw-bold'>
									<CENTER>
										
											<SELECT id="moto" name="moto" class="form-select" required>
												<OPTION disabled value="" selected hidden>Sélectionnez votre Moto</OPTION>
								...
								<INPUT type='submit' id='modifier' name='modifier' class='btn btn-primary text-light my-4' value='VALIDER'/>
										</FORM>
	...
	<a class="btn btn-primary text-light" href="Pass.pdf">TÉLÉCHARGER MON PASS PARTICIPANT</a>
	<br>
	<br>
	...
	<div class='row row-cols-1 row-cols-md-3'> <!-- BEGIN GRID -->
		...
		<?php
			...
			//on établit le roadbook des étapes effectuées
			...
			while($row = mysqli_fetch_assoc($result))
			{
				...
				Ville de départ : ".$row['depart']." (".$row['cp_depart'].")<br>
				Kilométrage effectué : ".$row['km']." km
				</p>
				</div>
				<div class='col-md-2'>
				<center>
				<!-- Button trigger modal delete-->
				<a href='#Delete".$row['id_voyage']."' role='button' data-toggle='modal' data-bs-toggle='tooltip' data-bs-placement='top' title='Effacer cette étape'>
				<img src='img/rally_delete-icon.svg'class='img-fluid' style='max-width:50px;'></a>
				<!-- Button trigger modal modify-->
				<a href='#Modify".$row['id_voyage']."' role='button' data-toggle='modal' data-bs-toggle='tooltip' data-bs-placement='top' title='Modifier cette étape'>
				...
				<div class='modal-content'>
				<div class='modal-header'>
				<h5 class='modal-title text-primary' id='Modaldelete'>Effacer l'étape vers ".$row['concess']." ?</h5>
				</div>
				<div class='modal-body text-primary fw-bold'>
				<CENTER>
				Voulez-vous vraiment effacer cette étape ?
				</CENTER>
				</div>
				...
				<div class='modal-content'>
				<div class='modal-header'>
				<h5 class='modal-title text-primary' id='exampleModalLabel'>Modifier l'étape vers ".$row['concess']."</h5>
				</div>
				<div class='modal-body text-primary fw-bold'>
				<CENTER>
				<FORM action='edit_stage.php?id_voyage=".$row['id_voyage']."' method='post' name='modify-etape' charset='UTF-8' class='needs-validation'>
				<LABEL for='date' class='form-label'>DATE DE L'ÉTAPE</LABEL>
				...
				<LABEL for='km' class='form-label'>KILOMÉTRAGE EFFECTUÉ</LABEL>
				<INPUT type='text' id='km' name='km' class='form-control' value='".$row['km']."'required/>
				<INPUT type='submit' id='modifier' name='modifier' class='btn btn-primary text-light btn-lg my-4' value='MODIFIER CETTE ÉTAPE'/>
				</FORM>
				</CENTER>
				</div>
				<div class='modal-footer'>
        ...
        echo"</div><br> <div class='row row-cols-1 row-cols-md-3'>";
			...
			while($row = mysqli_fetch_assoc($result))
			{
				...
				<div class='modal-content'>
				<div class='modal-header'>
				<h5 class='modal-title text-primary' id='exampleModalLabel'>Valider l'étape vers ".$row['concess']."</h5>
				</div>
				<div class='modal-body text-primary fw-bold'>
				<CENTER>
				...
				<INPUT type='submit' id='ajouter' name='ajouter' class='btn btn-primary text-light btn-lg my-4' value='VALIDER CETTE ÉTAPE'/>
				</FORM>
				</CENTER>
				</div>
				<div class='modal-footer'>
